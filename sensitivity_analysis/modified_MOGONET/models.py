...
class VCDN(nn.Module):
    def __init__(self, num_view, num_cls, hvcdn_dim):
        ...
        if num_cls == 1:
            ...
            # For regression: Concatenate views (Input dim = num_view)
            self.model = nn.Sequential(
                nn.Linear(num_view, hvcdn_dim),
                nn.LeakyReLU(0.25),
                nn.Linear(hvcdn_dim, 1)
            )
        else:
    ...
    def forward(self, in_list):
        ...
        if self.is_regression:
            # Concatenate the raw outputs from all views: [N, 1] + [N, 1] -> [N, 2]
            vcdn_feat = torch.cat(in_list, dim=1)
